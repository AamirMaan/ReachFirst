    ...
    if (event.currentTarget.value && event.currentTarget.value !== "") {
      ...
      for (var i = 0; i < searchRecord.length; i++) {
        if (searchRecord[i].name.toLowerCase().includes(searchInput)) {
          ...
          searchRecord[i].shoe_type.toLowerCase().includes(searchInput)
        ) {
          result.push(searchRecord[i]);
                      ...
                      <table className="table table-hover text-nowrap table-hover">
                        <thead>
                          <tr>
                            ...
                            <th>
                              Type
                              <i
                                onClick={() => handleAscSorting("shoe_type")}
                                className="fa fa-arrow-up fa-xs float-right"
                                style={{
                                  cursor: "pointer",
                                }}
                              />
                              <i
                                onClick={() => handleDesSorting("shoe_type")}
                                className="fa fa-arrow-down fa-xs float-right"
                                style={{
                                  cursor: "pointer",
                                }}
                        ...
                        <tbody>
                          {currentRecords.length > 0 &&
                            currentRecords.map((shoe, index) => {
                              return (
                                <tr key={index}>
                                  <td className="td-width">{shoe.name}</td>
                                  ...
                                  <td className="td-width">{shoe.color}</td>
                                  <td className="td-width">Rs:{shoe.price}</td>
                                  <td className="td-width">{shoe.size}</td>
                                  <td className="td-width">{shoe.stock}</td>
                                  <td>
                                    <i
                                      onClick={() => handleEdit(shoe.id)}
